...
export default class MittKontoScreen extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      url: (this.props.screenProps !== '' && !global.registered) ? (Platform.OS==='ios'?this.props.screenProps.replace('https','https:'):this.props.screenProps) : 'https://www.hemlyftet.se/account',
      loaded: false,
  ...
  _showProfilePage = () => {
    ...
    Linking.openURL('https://hemlyftet.se/account/profile'); 
  };
  ...
  _onNavigationStateChange = (webViewState) => { // you can also use navState
    ...
    let loggedIn = webViewState.url.search('https://www.hemlyftet.se/account') > -1;
    let profile = webViewState.url.search('https://www.hemlyftet.se/account/profile') > -1;
    let fActive = webViewState.url.slice(-1) == "#";
    let canGoBack = (webViewState.canGoBack && !(webViewState.url.search('https://www.hemlyftet.se/account/home') > -1));
    this.setState({
